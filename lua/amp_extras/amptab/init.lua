  ...
  current_cancel = client.complete(
    ...
    function(final_text)
      ...
      if M.config.debug then
        ...
        vim.notify(string.format("[AmpTab] Range: (%d,%d) to (%d,%d)", 
          ctx.range_start.row, ctx.range_start.col,
          ctx.range_end.row, ctx.range_end.col), vim.log.levels.DEBUG)
      end
      ...
      local end_line = vim.api.nvim_buf_get_lines(bufnr, clamped_end.row, clamped_end.row + 1, false)[1] or ""
...
function M.trigger(callback)
  ...
  current_cancel = client.complete(
    ...
    function(final_text)
      ...
      local display_text = cleaned_text:sub(prefix_match_len + 1, #cleaned_text - suffix_match_len):gsub("%s+$", "")
...
function M.setup(opts)
  ...
  if M.config.auto_trigger then
    vim.api.nvim_create_autocmd("CursorHoldI", {
      ...
      callback = function()
        ...
        if ghost.is_visible() then return end
  ...
  if M.config.default_keymaps then
    ...
    map("n", "gaa", function() M.accept() end, { desc = "AmpTab: Accept suggestion" })
    map("n", "gad", function() M.dismiss() end, { desc = "AmpTab: Dismiss suggestion" })
    map("n", "gat", function() M.trigger() end, { desc = "AmpTab: Trigger suggestion" })
