...
///
/// Auto-detects protocol format (JSON-RPC 2.0 or amp.nvim) and routes to appropriate handler.
/// Returns:
...
pub fn handle_text(text: &str) -> Result<Option<String>> {
    ...
    let value: Value = serde_json::from_str(text)
        .map_err(|e| AmpError::SerdeError(e))?;
    ...
    // Check if this is standard JSON-RPC (has "jsonrpc" field)
        ...
        Err(AmpError::Other("Unknown message format - missing clientRequest or jsonrpc field".into()))
    }
}

/// Handle amp.nvim wrapped request
fn handle_amp_request(client_req: ClientRequest) -> Result<ServerResponse> {
    let parsed = client_req.parse()
        .map_err(|e| AmpError::Other(e.into()))?;
...
mod tests {
    ...
    fn test_route_ide_edit_file() {
        ...
        let result = route_method("ide/editFile", json!({
            "path": path_str,
            "content": "hello"
        }));
    ...
    fn test_handle_amp_read_file_request() {
        ...
        let text = format!(r#"{{"clientRequest":{{"id":"req-456","readFile":{{"path":"{}"}}}}}}"#, path);
        let result = handle_text(&text).unwrap();
        ...
        assert_eq!(value["serverResponse"]["readFile"]["content"], "test content");
    }
    ...
    fn test_handle_amp_authenticate_request() {
        ...
        assert_eq!(value["serverResponse"]["authenticate"]["authenticated"], true);
    }

    #[test]
    fn test_handle_amp_get_diagnostics_request() {
        let text = r#"{"clientRequest":{"id":"req-diag","getDiagnostics":{"path":"/tmp/test.txt"}}}"#;
        let result = handle_text(text).unwrap();
